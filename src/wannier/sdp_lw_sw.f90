subroutine steepest_descent_lw_sw(i_step)
      use OMP_LIB
      use commvar
      implicit none
      ...
      real*8 :: delta_t_sw
      integer :: iter_eff, i_step, st, i_hit
      ...
      do
        if(count_iter.eq.0) then
          ...
          write(6,*) count_iter,OBJ_0,0,finish-start,delta_t_sw; call flush(6)
        end if
          ...
          !$OMP do schedule(dynamic)
           do j=1,ns
             do i=1,ns
                A(i,j) = delta_t_sw * sum(Q_A0(:,j,i)*(Q_A0(:,j,j)-Q_A0(:,i,i))-Q_A0(:,i,j)*(Q_A0(:,i,i)-Q_A0(:,j,j)))
             end do
           end do
          !$OMP end do
          !$OMP end parallel
      ...
      if(mod(count_iter,step_prnt_info)==0) then
        write(6,*) count_iter,OBJ_1,d_OBJ,finish-start,delta_t_sw; call flush(6)
      end if
...
end subroutine steepest_descent_lw_sw
